<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="age" name="age" type="number" class="block w-full mt-1" :value="old('age', $user->age)"
                required autofocus autocomplete="age" />
            <x-input-error class="mt-2" :messages="$errors->get('age')" />
        </div>
        <div>
            <x-input-label for="number" :value="__('number')" />
            <x-text-input id="number" name="number" type="text" class="block w-full mt-1" :value="old('number', $user->number)"
